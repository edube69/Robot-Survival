...
// Limite réduite à 10 comme demandé
const q = query(collection(db, 'highscores'), orderBy('score', 'desc'), fsLimit(10));

let firstLoad = true;
let lastTopScore = null;
let cachedDocs = []; // docs courants du top 10
let playerRank = null; // rang réel du joueur (peut être > 10)
let lastScoreMeta = safeParse(sessionStorage.getItem('lastScore')); // {score,name,kills,time,timestamp,rank?}
...
function renderLeaderboard() {
    ...
    // Si rang joueur > 10 on insère son rang réel + séparateur
    ...
    // Si joueur hors top, programmer la collapse après délai
    if (lastScoreMeta && playerRank && playerRank > 10) setTimeout(()=>collapseToTop10(), 5500); // 5.5s visibilité
}
...
onSnapshot(q, async snap => {
    ...
    // Calcul rang joueur si pertinent et pas encore fait
    if (lastScoreMeta && typeof lastScoreMeta.score === 'number' && !playerRank) {
        // Utiliser rank déjà mémorisé si présent
    ...
    if (snap.docs.length) {
        ...
        recordEl.textContent = topScore.toLocaleString(FRCA);
    ...
    // Confetti si nouveau record personnel top global
...
musicToggle.addEventListener('click', async ()=>{ if(useMp3Engine()){ if(bgm.paused){ try{await bgm.play();}catch{} musicToggle.textContent='Music: ON'; musicToggle.setAttribute('aria-pressed','true'); localStorage.setItem('bgmEnabled','1'); } else { bgm.pause(); musicToggle.textContent='Music: OFF'; musicToggle.setAttribute('aria-pressed','false'); localStorage.setItem('bgmEnabled','0'); } } else { if(!chip) chip=makeChipPlayer(); if(!chip.playing){ await chip.play(); musicToggle.textContent='Music: ON'; musicToggle.setAttribute('aria-pressed','true'); localStorage.setItem('bgmEnabled','1'); } else { chip.stop(); musicToggle.textContent='Music: OFF'; musicToggle.setAttribute('aria-pressed','false'); localStorage.setItem('bgmEnabled','0'); } } });
...
if(localStorage.getItem('bgmEnabled')==='1'){ musicToggle.textContent='Music: ON'; musicToggle.setAttribute('aria-pressed','true'); }

// ===== BACKGROUND + CONFETTI (inchangés) ===================================
...
// Correction du mapping pour l'apostrophe
